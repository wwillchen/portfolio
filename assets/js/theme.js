...
// Toggle between light and dark theme settings (binary only).
...
// Default is based on the user's system preference on first load.
let determineThemeSetting = () => {
  let themeSetting = localStorage.getItem("theme");
  if (themeSetting == "dark" || themeSetting == "light") {
    return themeSetting;
  }
  const userPref = window.matchMedia && window.matchMedia("(prefers-color-scheme: dark)");
  return userPref && userPref.matches ? "dark" : "light";
};

// Determine the computed theme, which can be "dark" or "light".
let determineComputedTheme = () => {
  // With binary toggle, computed theme equals the chosen setting.
...
let initTheme = () => {
  ...
  // No system mode; do not auto-switch on OS changes.
};
